package daily

import (
	"fmt"
	"sort"
)

/*

2410. Maximum Matching of Players With Trainers

You are given a 0-indexed integer array players,
where players[i] represents the ability of the ith player.
You are also given a 0-indexed integer array trainers,
where trainers[j] represents the training capacity of the jth trainer.

The ith player can match with the jth trainer if the player's ability is less than or equal to the trainer's training capacity.
Additionally, the ith player can be matched with at most one trainer, and the jth trainer can be matched with at most one player.

Return the maximum number of matchings between players and trainers that satisfy these conditions.


Example 1:

Input: players = [4,7,9], trainers = [8,2,5,8]
Output: 2
Explanation:
One of the ways we can form two matchings is as follows:
- players[0] can be matched with trainers[0] since 4 <= 8.
- players[1] can be matched with trainers[3] since 7 <= 8.
It can be proven that 2 is the maximum number of matchings that can be formed.

Example 2:
Input: players = [1,1,1], trainers = [10]
Output: 1
Explanation:
The trainer can be matched with any of the 3 players.
Each player can only be matched with one trainer, so the maximum answer is 1.


Constraints:

1 <= players.length, trainers.length <= 10^5
1 <= players[i], trainers[j] <= 10^9

Note: This question is the same as 445: Assign Cookies.

*/
/*
 What we need to do ?
  we need match players with trainers.
  - player[i] <= trainer[j] - must be.
  - only one match is acceptable for p[i] and t[j]
  - find a max possible match for players and trainers.
  Input: players = [4,7,9], trainers = [8,2,5,8]
 p[i]<=t[j]
 Burte force:
	1. while i<len(p) and j<len(t) check for each trainer.
	- if find match, count++ and break loop.
	- and move to next player[i] and start from next tainer[j]
tc: n*m;
sc: O(1)
2. How can we optimize?
	Since p[i] must be <= t[j] always we can sort arrays.
	1. sort arrays.
	take 2 pointers:
		i - players
		j - trainers.
	until j<len(t)
	start i=0; and go until p[i] > t[j] then.
	when p[i]<=t[j] then count++ and move to next player. i++
	always j++
	return
 tc: nlogn + nlogn + n + m
 sc: O(1)
 but maybe we dont need to sort trainers array.
*/

func matchPlayersAndTrainers(players []int, trainers []int) int {
	var count = 0
	sort.Ints(players)
	sort.Ints(trainers)
	fmt.Println(players, trainers)

	var i, j = 0, 0
	for i < len(players) && j < len(trainers) {
		if players[i] > trainers[j] {
			j++
		} else if players[i] <= trainers[j] {
			count++
			i++
			j++
		}
		fmt.Println(i, j, count)
	}
	return count
}
